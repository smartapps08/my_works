...
public class CompassView extends View {
	...
	protected void initCompassView() {
		...
		circlePaint.setColor(R.color.DeepBlue);
		...
		markerPaint.setColor(r.getColor(R.color.Red));
	...
	protected void onDraw(Canvas canvas) {
		...
		// skyPath.addArc(innerBoundingBox, -tiltDegree, (180 + (2 *
		// tiltDegree)));
		...
		for (double i = 0; i < 360; i += increment) {
			...
			PointF headStringCenter = new PointF(
					center.x - headStringWidth / 2, boundingBox.top + 1
							+ textHeight);
			if (headString.equalsIgnoreCase("W")) {
				// draw a linr from center
				markerPaint.setStrokeWidth(10);
				markerPaint.setColor(R.color.Orange);

				canvas.drawLine(center.x, center.y, headStringCenter.x,
						headStringCenter.y, markerPaint);
				markerPaint.setStrokeWidth(1);
			}
			if (i % increment == 0)
